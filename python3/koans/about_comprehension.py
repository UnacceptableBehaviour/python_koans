...
class AboutComprehension(Koan):
    ...
    def test_double_list_comprehension(self):
        ...
        #same
        comprehension = [ f'{egg} and {meat}' for egg in list_of_eggs for meat in list_of_meats]
    ...
    def test_creating_a_set_with_set_comprehension(self):
        comprehension = { x for x in 'aabbbcccc'}           # sets curly{}   tuples normal()
        # >>> type({ x for x in 'aabbbcccc'})
        # <class 'set'>
        # >>> { x for x in 'aabbbcccc'}
        # {'b', 'c', 'a'}
        # >>> 
        # >>> set('a','b','c')
